  ...
  return (
    ...
    <div className="relative select-none  min-h-86 h-2/3  min-w-64 w-92 outline-2 border-4 border-gris outline-gris overflow-hidden shadow-abberationrelief translate-y-6 " onClick={() => { handler(project.title) }}>
      ...
      <div className="absolute inset-0 duration-300 flex items-end justify-center opacity-0 hover:opacity-100 cursor-pointer transition-opacity  hover:translate-z-24">
        ...
        <p className="absolute text-white text-2xl pb-2 font-bold transition">{project.title}</p>
      </div>
    </div>
  ...
  return (
    <div className=" flex-1 max-w-2/3 perspective-normal perspective-origin-top-left  h-100vh overflow-hidden">
      <div className="absolute h-[100%] w-[95%]  translate-x-3 -translate-3 rotate-x-[2deg] rotate-y-[1deg] transform-3d justify-center overflow-hidden p-4">
        {(
          <div className="w-full h-full p-4 border-4 border-t-8 border-r-8 border-gris shadow-abberation bg-[#ababab] ">
            ...
            <div className="w-full h-full border-4 border-t-8 border-r-8 p-8 border-blur border-gris overflow-hidden bg-radial from-green-950 from-20% to-[#101010]">
              <div className="h-full w-full border-6 border-white flex shadow-neonblanc ">

              <div className="text-white flex h-16 text-5xl inset-y-32 right-32 absolute cursor-pointer text-shadow-neonblanc  " onClick={() => { setCurrentProject(null) }}>{"-->"}</div>
